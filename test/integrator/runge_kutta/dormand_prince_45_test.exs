defmodule Integrator.RungeKutta.DormandPrince45Test do
  ...
  describe "integrate/5" do
    test "gives the correct result for the van der pol function" do
      ...
      t = ~VEC[  19.93252120192793  ]f64
      x = ~VEC[  2.006431879061176e+00   9.819789615529644e-02  ]f64
      dt = ~VEC[  1.988834221159869e-03  ]f64
      t_next = Nx.add(t, dt)
      ...
      expected_t_next = ~VEC[  19.93451003614909  ]f64
      expected_x_next = ~VEC[  2.006622631532748e+00   9.362999867595886e-02  ]f64
      expected_x_est = ~VEC[   2.006622631532748e+00   9.362999867595506e-02  ]f64
      expected_k = ~MAT[
    ...
    test "gives the correct result when there are no existing k_vals" do
      ...
      t = ~VEC[  0.0  ]f64
      x = ~VEC[  2.0 1.0  ]f64
      dt = ~VEC[  1.463190090188842e-03  ]f64
      t_next = Nx.add(t, dt)
      ...
      expected_t_next = ~VEC[ 1.463190090188842e-03 ]f64
      expected_x_next = ~VEC[ 2.001457843004331  0.992694771307345 ]f64
      expected_x_est = ~VEC[  2.001457843004330  0.992694771307346 ]f64
    ...
    test "gives the correct result for the euler_equations" do
      ...
      expected_x_next = ~VEC[ 7.893280729541992e-02  9.968799385689663e-01  9.984099869704532e-01 ]f64
      expected_x_est = ~VEC[  7.893280780208246e-02  9.968799385539475e-01  9.984099869640367e-01]f64
    ...
    test "gives the correct result for the ballode high fidelity example - first integration" do
      t = ~VEC[  0.0  ]f64
      x = ~VEC[  0.0  20.0  ]f64
      dt = ~VEC[  0.001472499532027109  ]f64
      t_next = Nx.add(t, dt)
      ...
      expected_t_next = ~VEC[  1.472499532027109e-03  ]f64
      expected_x_next = ~VEC[  0.02943935535039590  19.98555477959081  ]f64
      #                     [0.0294393553503959,  19.985554779590814]       from this test
      #                     [0.02943934943567045, 19.985554782494084]       from actual simulation
      expected_x_est = ~VEC[   0.02943935535039590  19.98555477959081  ]f64
