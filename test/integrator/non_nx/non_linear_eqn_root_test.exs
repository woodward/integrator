defmodule Integrator.NonNx.NonLinearEqnRootTest do
  @moduledoc false
  use Integrator.TestCase
  import Nx, only: :sigils

  alias Integrator.DummyOutput
  alias Integrator.NonNx.NonLinearEqnRoot
  ...
  describe "sign" do
    test "is negative one for things less than one" do
      assert NonLinearEqnRoot.sign(-7.0) == -1.0
    end
    ...
    test "is zero for zero" do
      assert NonLinearEqnRoot.sign(0.0) == 0.0
    end
  end

  describe "unique/1" do
    test "returns all values if they are unique" do
      assert NonLinearEqnRoot.unique([3.3, 2.2, 1.1]) == [1.1, 2.2, 3.3]
    end

    test "only returns unique values sorted in ascending order" do
      assert NonLinearEqnRoot.unique([3.3, 2.2, 3.3]) == [2.2, 3.3]
    end
  end
end
