defmodule Integrator.MultiIntegrator do
  ...
  def integrate(ode_fn, event_fn, transition_fn, t_start, t_end, x0, opts) do
    ...
    integrate_next_segment(multi, :continue, ode_fn, transition_fn, Nx.to_number(t_start), t_end, x0, opts)
  end
  ...
  defp integrate_next_segment(multi, _status, ode_fn, transition_fn, t_start, t_end, x0, opts) do
    integration = Integrator.integrate(ode_fn, [t_start, t_end], x0, opts)
    new_t_start = List.last(integration.output_t)
    last_x = List.last(integration.output_x)

    new_x0 = transition_fn.(new_t_start, last_x)

    multi = multi |> update(new_t_start, last_x, new_x0, integration.terminal_event)
  ...
  @spec update(t(), Nx.t(), Nx.t(), Nx.t(), integration_status()) :: t()
  defp update(multi, new_t_start, last_x, new_x0, integration_event) do
    %{
      multi
